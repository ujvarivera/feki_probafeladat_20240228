...
export default function ProjectEditForm({ project, contactIdsOfProject, contacts }) {

    const { data, setData, post, processing, errors, reset, delete:destroy, put } = useForm({
        projectName: project.name,
        projectStatus: project.status,
        description: project.description,
        projectContacts: contactIdsOfProject,
        project_id: project.id
    });

    const addProject = (event) => {
        event.preventDefault();
        put(route('projects.update', { project: data.project_id }));
    };
